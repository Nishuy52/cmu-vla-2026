"""Forward-projection tests: round-trip pinning against tiling's backward mapping,
the VFOV gate, azimuth wrap, and the min-range gate."""
...
from tools.colored_cloud import project_points_to_pano
...
def test_roundtrip_recovers_bearing_elevation(yaw):
    ...
    # Azimuths spanning the strip; elevations inside the +-60 deg band.
    azis = np.linspace(-np.pi + 0.2, np.pi - 0.2, 13)
    els = np.linspace(np.deg2rad(-55), np.deg2rad(55), 7)

    for az_cam in azis:
        for el in els:
            bearing = tiling.wrap_pi(az_cam + yaw)  # camera az -> map bearing
            ...
            # Pixel -> camera ray -> map ray, using tiling's backward functions.
            ...
            # Within one pixel: 2pi/1920 ~ 3.3 mrad az, 120deg/640 ~ 3.3 mrad el.
...
def test_vectorized_azimuth_matches_scalar(yaw):
    ...
    for b, v in zip(bearings, vec):
        ...
        # Both wrapped to (-pi, pi]; compare on the circle.
...
def test_vfov_gate_never_clamps():
    ...
    # Just inside the band stays valid.
...
def test_azimuth_wrap_at_seam():
    """A camera azimuth just left/right of column 0 (the yaw+pi seam) lands at
    col 0 / col 1919 as azimuth wraps around the strip."""
    odom = OdomState(t=0.0, x=0.0, y=0.0, z=0.0, yaw=0.0)
    apex = (0.0, 0.0, 0.0)
    eps = 2.0 * np.pi / tiling.PANO_WIDTH * 0.25  # a quarter column

    # Camera azimuth just below +pi -> map bearing just below +pi -> near column 0.
    for az_cam, want_col in [(np.pi - eps, 0), (-np.pi + eps, tiling.PANO_WIDTH - 1)]:
        bearing = tiling.wrap_pi(az_cam)  # yaw = 0
