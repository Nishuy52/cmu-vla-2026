...
#
#   tools/cluster/live_run/push_and_verify.sh [scene] ["question"]
#
...
echo "== submit"
JOB=$(ssh xlogin "cd ~ && QUESTION=\"$QUESTION\" SCENE_DIR=~/scenes/$SCENE sbatch --export=ALL ~/cluster_verify_run.sbatch" | grep -oE '[0-9]+')
echo "SUBMITTED job $JOB (scene=$SCENE)"
