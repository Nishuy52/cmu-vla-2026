...
| geometry.colour_dominance_floor | 0.50 | frac | Min bin fraction for a cross-hue bridged colour (red→maroon) or a black/white luminance-bridged `gray` bin to count (issue #12) | Battery-validated (rejects hb2 18% 3rd-bin maroon, admits 78%) | function param (via `_attrs_match`→`_colour_present`) | M |
