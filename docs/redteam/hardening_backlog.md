...
| CV sweep | **RUNNING (15 Jul)**: launched after the instrument-endogeneity fix (frontier critique meth-F1; cvsweep IF rubric geometry/alignment frozen at defaults, `81f2a2a`) and a green full gate (1090 passed, 5 skipped). Output: `reports/cvsweep_2026-07-15/` (disk-resume; BelowNormal priority). **Adoption gated** per `docs/tasks/T11-if-leg-threading/critique/adjudication.md`: whole-config beats baseline by >1 six-pt-question-equivalent, no per-type regression, identical instrument footprint, per-key ablation ≥2 questions, IF-driven keys provisional-until-real-sim |
