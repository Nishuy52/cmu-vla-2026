...
Sources: DD-A1..A8 (generation-code-verified priors), NUM-F2 (empirical: 0/25 real pillows pass `on(sofa/bed)`; 0/5 pictures pass `above(bed)`). Pin the *forms* the sweep cannot discover: `on` = anchor z-band (not top-face-only) + IoM-over-min + anchor-larger gate; `between` = strict betweenness (0<t<1) + symmetry gate; `next_to`/"beside"/"adjacent to" alias to `near`; `with` ≡ inverse-`on`; `under` gains the tuck-under branch (class-gated); `above` = IoM ≥ 0.5 prior; wire or drop the dead `above_gap_max`. Leave coefficients to the (post-H2) sweep per the DD pin-vs-sweep table. **Verdict: accepted — a mismatch with the code that generated the questions is a scoring bug, not a style choice.** Watch: `on` also feeds OR/IF anchoring — regression-check those goldens.

### H6. Perception wiring is a gated deliverable, not a comment — **~48 pts if unshipped, cost: seam S now / integration L at Ubuntu**
Sources: SYS-F1. The committed adapter answers every question from `BasicSceneIndex([])`; the pano is latched and never consumed; the Dockerfile's detector install is commented out. Acknowledged Phase-2 work — the *defect* is that nothing gates submission on it and every smoke test passes with the stub. **Verdict: accepted.** Fix now (Windows): the `PerceptionPipeline` seam in the adapter following the `runner/single.py` scripted pattern, a loud startup log when the index is the stub, and a phase2_playbook gate item "instances_tracked > 0 on a live scene before any submission". Integration itself stays Ubuntu-gated.
